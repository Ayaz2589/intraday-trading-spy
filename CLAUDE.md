...
**Active plan**: [specs/021-paper-trading/plan.md](specs/021-paper-trading/plan.md) — **planned, implementation pending** (live paper trading via the Alpaca PAPER account + /trade page: multi-day automation session — asyncio task, campaign-style, one-running unique index, restart → `interrupted` never silent-resume — reusing strategy/risk/indicators/clock verbatim on live 5m bars (1m websocket aggregated; REST warmup so mid-session starts get full-session VWAP/OR); entries are BRACKET orders so stop+target rest broker-side from acceptance; paper endpoint hard-asserted, `live_auto_enabled` untouched; four append-only `paper_*` tables (migration 0129) strictly SEPARATE from the runs/Insights archive; risk-gated manual orders; klinecharts live views 1m/5m/1d/30d + VWAP (none on 30d, labeled); sizing from config `account_value` for archive comparability; forward record = the new out-of-sample now that the lockbox is spent/burned — Exp 011).
