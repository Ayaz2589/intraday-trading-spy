...
**Active plan**: [specs/022-historic-trade-replay/plan.md](specs/022-historic-trade-replay/plan.md) — **planned** (Historic Trade page under `/trade` at `/trade/historic`: replays a chosen past SPY session bar-by-bar from stored 5m history at user-selectable speed (1s…1hr = sim market-seconds per real second; continuous sim-clock, bars surface at 5m boundary crossings, no sub-bar synthesis), with manual buy/sell + watchable strategy automation. **Drives the BACKTEST primitives** (strategy→risk→`broker/paper.py` honest-cost fills, next-bar-open, stop-first bracket) NOT the live engine — SC-004 demands automation match a backtest 100%; reuses the live page's cockpit/journal-vocabulary/API-shape for the experience. **Ephemeral in-memory only** (REPLAY_RUNNING registry mirroring live's RUNNING) — zero DB tables/migration, which makes archive/`paper_*` leakage structurally impossible (SC-005). New `replay/` package + `/api/replay/*` router + `HistoricTradePage` reusing LiveChart/ForwardPerformance/LiveJournalTable/AccountPanel/ManualOrderForm; no new deps).
