...
**Active plan**: [specs/019-auto-research/plan.md](specs/019-auto-research/plan.md) — in planning (automated strategy research: authenticated research CLI for every pipeline step via the email-OTP session flow + make wrappers; server-side auto-research campaigns — BackgroundTask over a new `research_campaigns` table, migration 0126 — cycling data-freshness → walk-forward → Bonferroni-tightened pooled gate (level 1−α/k by family trial count) → deterministic recommendation actuation, halting only at ready_for_lockbox / stop_tuning / budget_exhausted / cancelled / failed and NEVER touching the lockbox; Validation-page Auto-research section + campaign detail route).
