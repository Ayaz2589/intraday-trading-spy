...
| **015 — Monte Carlo path-risk** ✅ **DONE** *(inserted at spec time; numbers below shifted +1)* | on-demand Monte Carlo on any run's trades: shuffle drawdown/streak/underwater distributions vs observed, bootstrap forward cone + terminal percentiles, risk-of-ruin per threshold; run-detail panel w/ caveat-unless-provably-OOS banner; zero migrations, zero new deps | The validation engine answered "is the edge real?" but not "what's the risk of riding it?" — path stats gate any paper/live decision | 011, 014 (child runs make every study window a simulatable run) |
| **016 — Insights + pooled gate + Claude narrative** *(scope finalized at brainstorm; retention/rejection-mining/sensitivity-archive deferred)* | pooled study gate (productized wf-rr3 lockbox gate, persisted on the study, fast+full modes); Insights page (edge time-series + per-config window distribution over the OOS archive); advisory Claude narrative on both surfaces (cited findings, snapshot-pinned, billing-pause switch) | The 2026-06-05 wf-rr3 gate had to be run ad-hoc — the decision number didn't exist in the product | 011, 014, 015 |
| **017 — Claude experiment drafts** | whitelist-sanitized knob suggestions in analyses (NEW knob registry w/ bounds); experiment cards render `knob → value` chips + "Draft config →"; transient URL draft → badged prefilled panel on Strategies; human-gated create w/ durable provenance (migration 0124: `configs.description`) | Claude's knob-class experiments required manual transcription into configs; Constitution-II guardrails (no LLM write path) are the spine | 016 |
| **017 (optional) — Research/Learn UI lanes** | reorganize nav/IA into "Backtest / Learn" and "Validate / Research" sections over the shared core | Clarity; can be folded into earlier features | 012, 014 |
...
done — inserted at spec time) → 014 (study runs, done) → 015 (monte carlo
path-risk, done — inserted at spec time) → 016 (insights) → [017 lanes]**. The
roadmap's §10 feature map carries the same numbering.
