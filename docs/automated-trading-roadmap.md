...
| `013` | **Phase 2 — study child-run persistence + drill-down** | orchestrator writes each window/grid eval as a study-tagged run; study-detail ↔ run-detail; significance attaches to a window | Not started (next; closes the run/study seam) |
| `014` | **Phase 2 — insights** | cross-run insights/aggregation API + views (per-config distribution, sensitivity across the archive, edge time-series, rejection mining) + the `008` soft-delete retention prereq | Not started (deferred from `011`; needs `013`'s child runs) |
