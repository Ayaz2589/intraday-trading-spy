...
> **Status: ✅ ENGINE BUILT, TESTED & MERGED** — feature `011-validation-engine` (merged to main). Backend + frontend complete and green; cloud migrations `0110`–`0112` applied. Train/validation/lockbox split, walk-forward, parameter-sensitivity surface, bootstrap + random-entry-permutation significance, and the one-shot lockbox gate are all implemented and launchable via `/api/validation/*` and the Validation UI. **Follow-on `012-config-management` is ✅ done** — first-class named configs (create/duplicate/rename/delete/activate) + a SPY-workable default (cap=400) so the engine has real configs to compare; this fixed the 0-trade wall that made every study look identical. Next: `013` (study child-run persistence + drill-down). **The exit gate below is *operational*, not code** — it is met only by actually running studies and seeing whether a candidate config survives (or nothing does, which is a valid result). Remaining before the gate can be evaluated: a live end-to-end run on the cloud data + a perf pass.
...
| `011` | **Phase 2 — validation** | train/validation/lockbox split, walk-forward, robustness/sensitivity surface, bootstrap + random-entry-permutation significance, one-shot lockbox gate | **✅ Done & merged to main**. Backend + frontend complete; cloud migrations `0110`–`0112` applied. Decided the **011/012 split** (this is the validation engine; first-class configs → `012`; insights/aggregation + `008` soft-delete → `014`) and kept it **evaluate-and-report only** (Principle II — no auto-optimizer; a guard test asserts no validation path reaches live trading). FR-005 child-run drill-down deferred to `013`. Exit gate is operational: pending a live e2e run + perf pass. |
| `012` | **Phase 2 — first-class configs** | create/duplicate/rename/delete/activate named configs; presets as starting points; config-management UI on the Strategies page; SPY-workable default (cap=400) + presets that all trade; pickers pre-select the active config | **✅ Done**. Migrations `0120`–`0122` applied (one-active-per-user index, `runs.config_id` nullable + `ON DELETE SET NULL`, seed cap=400). Backend CRUD + presets loader + API; frontend config-manager; fixed the 0-trade `position_size_cap` wall. Edit-isolation + lifecycle-journaling + full e2e verified live against cloud. The previously-pencilled "012 insights/aggregation" shifted to **`014`** (see [research-tooling-uplift.md](research-tooling-uplift.md)). |
| `013` | **Data observability** | Data-page uplift: backfill job history (failures stay visible w/ reasons), cache summary strip (totals/sources/last-updated), year×month completeness heatmap w/ exact missing-day hover, light lineage line | **✅ Done**. `GET /api/bars/stats` + `GET /api/bars/backfill` (history); pure `month_stats` derivation (holiday-vs-gap unit-tested); no DB migration (R8 psycopg aggregate); 3 new `?` tooltips; verified live (168k bars, 102 months, 0 missing sessions). Spec'd from the brainstormed design `docs/superpowers/specs/2026-06-04-data-observability-design.md`. |
| `014` | **Phase 2 — study child-run persistence + drill-down** | orchestrator writes each window/grid eval as a study-tagged run; study-detail ↔ run-detail; significance attaches to a window | **✅ Done**. Zero migrations (0111/0112 columns finally written). In-memory `build_run_payload` shared with the file path (parity-locked); per-eval push w/ `spec_hash` dedup + fail-soft `persisted` flag; lockbox child + ledger link; runs list hides children; child runs badge back to their study; **Re-run study** clones any study into a drillable one; study detail page redesigned (expandable IS/OOS window rows, sensitivity points table). Also fixed the latent API summary-write crash (`mode="json"`) that had failed every API backtest with trades since `010`. |
| `015` | **Phase 2 — insights** | cross-run insights/aggregation API + views (per-config distribution, sensitivity across the archive, edge time-series, rejection mining) + the `008` soft-delete retention prereq | Not started (deferred from `011`; needs `014`'s child runs) |
